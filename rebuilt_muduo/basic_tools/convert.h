...
inline const char digitsHex[] = "0123456789ABCDEF";
static_assert(sizeof digitsHex == 17, "wrong number of digitsHex");

/*
@brief 将数字转换为字符串，返回转换的字符串长度
@param buf 接收缓冲区
@param value 待转换的数字
@return 转换后的字符串长度
 */
...
/*
    @brief 将数字转换为16进制字符串，返回转换的字符串长度
    @param buf 接收缓冲区
    @param value 待转换的数字
    @return 转换后的字符串长度
*/
...
/*
 ...
 [1.00E, inf)
*/
inline std::string formatSI(int64_t s)
{
...
/*
 ...
 [1.00Mi, 9.99Mi]
*/
inline std::string formatIEC(int64_t s)
{
  double n = static_cast<double>(s);
  char buf[64];
